...
public void run () {
 ...
 for (int I=1;  I<= 4; I++) {
   ...
   Synch.totalCars ++;//count cars at every state.
   ...
   Synch.totalCars ++;
   //westbound cars have to wait.
   if ((TrafficLight.getLight().getCurrent() == "RED")||(TrafficLight.getLight().getCurrent() == "EAST_GREEN")) {
	   Synch.westBoundCar.acquire(); //acquire semaphores.
	   Synch.westCarWaiting ++; //count the number of waiting cars in eastbound 
   }
   Synch.totalCars --;
   ...
   // *** light is red..
   Synch.totalCars ++;
   //eastbound cars have to wait.
   if ((TrafficLight.getLight().getCurrent() == "RED")||(TrafficLight.getLight().getCurrent() == "WEST_GREEN")) {
	   Synch.eastBoundCar.acquire();
	   Synch.eastCarWaiting ++;	//count total waiting eastbound cars.   
   }
   Synch.totalCars --;
